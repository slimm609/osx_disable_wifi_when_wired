...
# Grab the names of the adapters. We assume here that any ethernet connection name ends in "LAN"
...
for eth_name in ${eth_names}; do
    if [[ "${eth_name}" -ne "" ]] && [[ "$(ifconfig "${eth_name}" | grep "status: active")" -ne "" ]]; then
        eth_status="On"
    fi
done
...
if [[ "${prev_air_status}" -ne "${air_status}" ]] || [[ "${prev_eth_status}" -ne "${eth_status}" ]]; then
    if [[ -s "./statusChanged.sh" ]]; then
        "./statusChanged.sh" "${eth_status}" "${air_status}" &
    fi
fi

# Determine whether ethernet status changed
if [[ "${prev_eth_status}" -ne "${eth_status}" ]]; then
...
# If ethernet did not change
    ...
    if [[ "${prev_air_status}" -ne "${air_status}" ]]; then
